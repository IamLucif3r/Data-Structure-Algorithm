import java.util.*;
public class knights_move_optimized {
    ...
    public static void main(String[] args) {
        ...
        //     System.out.println(i.x+" : "+i.y);
            ...
            board[x][y] =1;
            List<Cell> possibilities = possibleMoves(x, y);
            Cell minimum= possibilities.get(0);
            for(Cell i :possibilities)
            {
                if(possibleMoves(i.x, i.y).size()<= possibleMoves(minimum.x, minimum.y).size())
                {
                    minimum.x =i.x;minimum.y = i.y;
                }
